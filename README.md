...
- Tapping a future trimester chip advances/creates it immediately (no confirmation). If that gets tapped by mistake and "current" ends up ahead of the real date, the Dashboard shows a "reset to today" warning that rolls it back and discards the accidental trimester(s) — any real client data in trimesters up to and including today is untouched.
